...
# spark-submit --master=local[4] spark-routingtable.py --start-time=1451606400 --end-time=1451779200 -t updates -c route-views.sg
#
...
def analyze(start_time, end_time, data_type, outdir,
    ...
    conf = SparkConf()\
        .setAppName("RoutingTable.%s.%s-%s" % (data_type, start_str, end_str))\
    ...
    outfile = "%s/bgpstream-routingtable.%s.%s-%s.csv" %\
              (outdir, data_type, start_str, end_str)

    with open(outfile, 'wb') as csvfile:
        w = csv.writer(csvfile)
        w.writerow(["Time", "Collector", "Peer", "#Pfxs_v4", "#Pfxs_v6", "#ASNs_v4", "#ASNs_v6"])

        # write out the per-peer statistics
        for key in final_time_collector_peer:
            (ts, coll, peer) = key
            (pfxs_v4, pfxs_v6, asns_v4, asns_v6) = final_time_collector_peer[key]
        ...
        for key in final_time_collector:
            (ts, coll) = key
            (pfxs_v4, pfxs_v6, asns_v4, asns_v6) = final_time_collector[key]
